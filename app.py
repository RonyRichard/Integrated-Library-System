...
def connect_to_database():
    return mysql.connector.connect(
        host="localhost",
        user="root",
        passwd="0612049471",  # Replace with your actual password
        database="library"
    )
...
def login():
    ...
    if account:
        ...
        if account[3]:  # Check if user is admin
...
def show_books_checked_out():
    ...
    cursor.execute("""
        ...
        WHERE cl.library_card_no = %s AND b.checked_out = TRUE
    """, (current_user[0],))
    ...
    if not books:
        messagebox.showinfo("Books Checked Out", "No books currently checked out.")
        return

    books_window = tk.Toplevel(root)
    books_window.title("Books Checked Out")
...
def check_out_book():
    ...
    if book_id is None:  # Check if the dialog was canceled
        return
    ...
    try:
        ...
        if book[0]:  # Book is already checked out, check it in
        ...
        else:  # Book is not checked out, proceed to check it out
            if current_user[3]:  # Check if the user is admin
                library_card_no = simpledialog.askstring("Input", "Enter Library Card Number for user:")
                if library_card_no is None:
                    return
            else:
            ...
            cursor.execute("""
                ...
                VALUES (%s, %s, %s, %s, CURDATE(), NULL)
            """, (library_card_no, account[0], account[1], book_id))
            cursor.execute("UPDATE Books SET checked_out = TRUE WHERE book_id = %s", (book_id,))
            db.commit()
            messagebox.showinfo("Success", f"Book '{book_id}' checked out successfully.")
...
def main_window():
    ...
    ttk.Button(user_frame, text='Add Book', command=add_books).pack(fill=tk.X, pady=5)  # Add Book button on the main menu
